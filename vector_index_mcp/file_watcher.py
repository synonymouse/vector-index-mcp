...
class FileWatcher:
    ...
    def __init__(
        ...
        logging.info(f"FileWatcher initialized for project: {self.project_path}. Ignoring {len(patterns)} patterns.")
    ...
    def _get_last_modified(self, file_path: str) -> float:
        ...
        try:
            ...
            logging.warning(f"File not found when getting last modified time: {file_path}")
            return 0
        except Exception as e:
            logging.error(f"Error getting last modified time for {file_path}: {e}", exc_info=True)
            return 0
    ...
    def _should_ignore(self, path: str) -> bool:
        ...
        if self.abs_lancedb_path_to_ignore and real_event_path.startswith(self.abs_lancedb_path_to_ignore):
            logging.debug(f"Ignoring path '{real_event_path}' as it is within the LanceDB directory '{self.abs_lancedb_path_to_ignore}'.")
            return True
        
        absolute_path = Path(path).resolve()
        if absolute_path.is_dir(): # Always ignore directories themselves, only process files
        ...
        try:
            ...
            if is_ignored:
                logging.debug(f"Ignoring path '{path}' due to match in ignore patterns (relative: '{relative_path}').")
            return is_ignored
        except ValueError:
    ...
    def _process_and_index_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            if not file_hash: # Hash calculation failed (e.g. file disappeared)
                logging.warning(f"Could not calculate hash for {file_path}, skipping processing.")
            ...
            chunks = chunk_content(content) # Assumes chunk_content handles empty content gracefully
            total_chunks = len(chunks)

            if total_chunks == 0:
                logging.info(f"File '{file_path}' is empty or resulted in no processable chunks. Removing from index if present.")
            ...
            logging.info(f"Successfully indexed {total_chunks} chunks for file: {file_path}")
            return True

        except FileNotFoundError:
            logging.warning(f"File not found during processing (it may have been deleted rapidly): {file_path}")
        ...
        for root, _, files in os.walk(self.project_path, topdown=True):
            ...
            for file_name in files:
                ...
                if known_info:
                    ...
                    if current_hash == known_info["hash"] and current_modified == known_info["last_modified"]:
                        logging.debug(f"Skipping unchanged known file during initial scan: {file_path}")
                        processed_files_count +=1 # Count as "processed" in the sense of "checked"
                        continue
                
                logging.debug(f"Initial scan: Processing file {file_path}")
                if self._process_and_index_file(file_path):
                    processed_files_count += 1
        logging.info(f"Initial scan complete. Processed (checked or indexed) {processed_files_count} files.")
    ...
    def process_modification(self, file_path: str):
        ...
        if not current_hash: # Hash calculation failed (e.g., file deleted quickly after modify event)
            logging.warning(f"Hash calculation failed for modified file {file_path}. It might have been deleted. Removing if known.")
            if file_path in self.known_files:
        ...
        if not known_info:
            ...
            logging.info(f"Change detected in {file_path} (Hash or MTime mismatch). Re-indexing...")
            needs_reindex = True
        else:
            logging.debug(f"No significant change (hash and mtime match) detected for {file_path}. Skipping re-index.")
        ...
        if needs_reindex:
            try:
                ...
                logging.error(
                    f"Error during re-indexing of modified file {file_path}: {e}", exc_info=True
                )
    ...
    def process_deletion(self, file_path: str):
        ...
        if file_path in self.known_files:
            ...
            logging.debug(f"Deletion event for an untracked or already removed file: {file_path}")
...
class ProjectEventHandler(FileSystemEventHandler):
    ...
    def on_moved(self, event):
        ...
        if not event.is_directory: # If a file was moved
            ...
            logging.info(f"Directory moved: {event.src_path} -> {event.dest_path}. Individual file events will be processed if generated by OS.")
