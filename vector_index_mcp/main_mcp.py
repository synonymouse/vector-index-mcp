...
async def lifespan_manager(mcp_app: FastMCP):
    ...
    log.info(f"Lifespan: Initializing MCPServer with project_path: {project_path_from_cli}")
...
async def trigger_index_tool(force_reindex: bool = False) -> dict:
    ...
    try:
        ...
        log.info(f"trigger_index_tool: Triggering scan with force_reindex={force_reindex}")
        await mcp_server._scan_project_files(project_path=mcp_server.project_path, force_reindex=force_reindex)
        return {"content": [{"type": "text", "text": "Indexing successfully triggered."}], "isError": False}
...
async def get_status_tool() -> dict:
    ...
    try:
        ...
        return {"content": [{"type": "text", "text": json.dumps(status_data)}], "isError": False}
    except Exception as e:
...
def main():
    ...
    """
    if len(sys.argv) < 2:
        log.critical("Error: Project path not specified.") # Changed to critical as it prevents server start
