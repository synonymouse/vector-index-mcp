...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(env_file=_env_file_value, env_file_encoding='utf-8', extra='ignore')
    ...
    ignore_patterns: List[str] = [".*", "*.db", "*.sqlite", "*.log", "node_modules/*", "venv/*", ".git/*"]
...
def get_vector_index_settings() -> Settings:
    ...
    if _settings_instance is None:
        ...
        if env_project_path:
            ...
            _settings_instance.project_path = os.path.abspath(_settings_instance.project_path)

        env_lancedb_uri = os.getenv("LANCEDB_URI")
        if env_lancedb_uri:
            if not os.path.isabs(env_lancedb_uri):
                _settings_instance.lancedb_uri = os.path.join(_settings_instance.project_path, env_lancedb_uri)
            else:
                _settings_instance.lancedb_uri = env_lancedb_uri
        else:
             if not os.path.isabs(_settings_instance.lancedb_uri):
                _settings_instance.lancedb_uri = os.path.join(_settings_instance.project_path, _settings_instance.lancedb_uri)
